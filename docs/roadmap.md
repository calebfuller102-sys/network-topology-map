...
| Monitor editor and per-check diagnostics UI | Phase 7 | Implemented locally; automated checks and an isolated loopback browser check are recorded in `docs/phase-7.md` | Owner can configure checks, trigger a manual run, and understand each check's latest result in the inspector |
