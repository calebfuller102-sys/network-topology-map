...
docker buildx build --pull --platform "$platform" --load \
  --tag ghcr.io/calebfuller102-sys/network-topology-map-api:0.1.530 --file "$repo_root/backend/Dockerfile" \
  "$repo_root/backend"
docker buildx build --pull --platform "$platform" --load \
  --tag ghcr.io/calebfuller102-sys/network-topology-map-web:0.1.530 --file "$repo_root/frontend/Dockerfile" \
  "$repo_root"
...
for image in \
  ...
  ghcr.io/calebfuller102-sys/network-topology-map-web:0.1.530; do
